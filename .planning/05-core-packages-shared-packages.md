---
title: "Shared Packages — Deep Dive"
...
### 5.4 `packages/ui` — Shared Design System
...
#### 5.4.1 Package Structure (`packages/ui/`)
...
@import "@suite/ui/styles/globals.css";
```
...
```typescript
transpilePackages: ['@suite/ui'],
```

For a Vite app, add to `vite.config.ts`:

```typescript
optimizeDeps: { include: ['@suite/ui'] },
```
...
```bash
pnpm nx g @myorg/ui:add-component button card dialog
```

#### 5.4.5 AI Agent Rules (AGENTS.md)

```markdown
## @suite/ui — Rules for AI Agents
...
3. **Tailwind classes should be composed via `cn()` helper.** Already exported from `@suite/ui`.
...
| `ui` | Shared components, Tailwind theme, design tokens | Shadcn/ui + Tailwind v4 | N/A (UI only) | All frontend SPAs |
