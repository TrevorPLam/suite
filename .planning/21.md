...
| Shared packages created (`crypto`, `db`, `auth`, `ui`, `shared-kernel`) | 🔄 | Each has `package.json` and exports a public API |
...
5. **Create `packages/ui`** — Shadcn components, Tailwind config
...
| `import { Button } from '../components/ui/button'` | `import { Button } from '@suite/ui'` |
