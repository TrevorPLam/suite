...
### [x] UI-005: Integrate axe-core for Accessibility Testing
...
**Action**: Added vitest-axe to devDependencies for Vitest compatibility (instead of jest-axe).
**Validate Command**: `pnpm --filter @suite/ui typecheck`

#### ✅ UI-005-02: Configure Vitest for axe-core
**Assigned To**: AGENT
**Target File**: `packages/ui/vitest.config.ts`, `packages/ui/vitest.setup.ts`
**Action**: Created vitest.setup.ts with jest-dom and vitest-axe matchers. Updated vitest.config.ts to include setupFiles. Added scrollIntoView mock for Radix UI jsdom compatibility.
**Validate Command**: `pnpm --filter @suite/ui test`

#### ✅ UI-005-03: Add accessibility test to Button component
**Assigned To**: AGENT
**Target File**: `packages/ui/src/components/ui/button.test.tsx` (created)
**Action**: Created button.test.tsx with accessibility tests for default, primary, secondary, danger, and disabled variants. Tests keyboard accessibility and accessible names.
**Validate Command**: `pnpm --filter @suite/ui test button.test.tsx`

#### ✅ UI-005-04: Add accessibility test to Input component
**Assigned To**: AGENT
**Target File**: `packages/ui/src/components/ui/input.test.tsx` (created)
**Action**: Created input.test.tsx with accessibility tests for default, error, success, disabled, and required states. Tests label associations and keyboard navigation.
**Validate Command**: `pnpm --filter @suite/ui test input.test.tsx`

#### ✅ UI-005-05: Add accessibility test to Dialog component
**Assigned To**: AGENT
**Target File**: `packages/ui/src/components/ui/dialog.test.tsx` (created)
**Action**: Created dialog.test.tsx with accessibility tests for open/closed states, focus management, ARIA attributes, and trigger button.
**Validate Command**: `pnpm --filter @suite/ui test dialog.test.tsx`

#### ✅ UI-005-06: Add accessibility tests to remaining components
**Assigned To**: AGENT
**Target File**: `packages/ui/src/components/ui/` (card.test.tsx, badge.test.tsx, select.test.tsx, textarea.test.tsx, skeleton.test.tsx)
**Action**: Created accessibility tests for Card, Badge, Select, Textarea, and Skeleton components. Each test verifies no a11y violations and tests all variants.
**Validate Command**: `pnpm --filter @suite/ui test`

#### ✅ UI-005-07: Update index.test.tsx with a11y test
**Assigned To**: AGENT
**Target File**: `packages/ui/src/index.test.tsx`
**Action**: Added integration-level accessibility test rendering all components together to catch integration-level a11y issues.
...
**Action**: Updated README.md with accessibility testing section including how to run tests, writing guidelines, common violations, and jsdom mocking requirements.
**Validate Command**: No validation needed

**Implementation Notes**:
- Used `vitest-axe` instead of `jest-axe` for Vitest compatibility
- Added `@ts-expect-error` comments to suppress TypeScript type errors for vitest-axe matchers (runtime types not fully recognized)
- Mocked `Element.prototype.scrollIntoView` in vitest.setup.ts for Radix UI jsdom compatibility
- All 61 tests passing (9 test files)
- Lint passes with no errors
- Typecheck has pre-existing errors in i18n files (unrelated to UI-005)
